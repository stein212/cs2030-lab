...
class BusService {
    ...
    public CompletableFuture<Set<BusStop>> getBusStops() {
        return BusAPI.getBusStopsServedBy(serviceId).thenApply(this::processBusStopResponse);
    }

    private Set<BusStop> processBusStopResponse(String busStopResponse) {
        Scanner sc = new Scanner(busStopResponse);
    ...
    public CompletableFuture<Set<BusStop>> findStopsWith(String name) {
        return getBusStops().thenApply(bs -> filterStopsWith(bs, name));
    }

    private Set<BusStop> filterStopsWith(Set<BusStop> busStops, String name) {
        return busStops.stream().filter(stop -> stop.matchName(name)).collect(Collectors.toSet());
    }
